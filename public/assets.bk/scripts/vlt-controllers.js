		...
		init: function () {
			...
			el.pagepiling({
				...
				onLeave: function (index, nextIndex, direction) {
                    ...
                    if(direction == "down" && nextIndex == 2 && typeof map !== 'undefined'){
                        map.setZoom(2);
                        // animateMapZoomTo(map, 3)
                        setTimeout(function(){
                            // map.setZoom(10)
                            animateMapZoomToT(map, 10)
                        }, 3000);
                    }
					vlthemes_page_brightness();
...
function animateMapZoomToT(map, targetZoom) {
    var currentZoom = arguments[2] || map.getZoom();
    if (currentZoom != targetZoom) {
        google.maps.event.addListenerOnce(map, 'zoom_changed', function (event) {
            animateMapZoomToT(map, targetZoom, currentZoom + (targetZoom > currentZoom ? 1 : -1));
        });
        setTimeout(function(){ map.setZoom(currentZoom) }, 220);
    }
}
/***********************************************
 * MENU OFFCANVAS
